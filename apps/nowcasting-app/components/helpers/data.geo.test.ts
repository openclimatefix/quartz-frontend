...
//////////////////////////////////////////////////////////////////////////////
// aggregateTruthData — lives in components/charts/gsp-pv-remix-chart/
// use-get-gsp-data.ts, not in helpers/data.ts, and is not exported. It is
// exercised here through its hook so that audit B5 is pinned.
//////////////////////////////////////////////////////////////////////////////
jest.mock("@sentry/react", () => ({ captureMessage: jest.fn() }), { virtual: true });
jest.mock("../hooks/useLoadDataFromApi", () => ({ useLoadDataFromApi: jest.fn() }));

const { renderHook } = require("@testing-library/react");
const { useLoadDataFromApi } = require("../hooks/useLoadDataFromApi");
const useGetGspData = require("../charts/gsp-pv-remix-chart/use-get-gsp-data").default;

describe("aggregateTruthData (via useGetGspData)", () => {
  const mockedLoad = useLoadDataFromApi as jest.Mock;

  const descendingRows = (): YieldRow[] => [
    {
      datetimeUtc: "2023-12-05T13:00:00+00:00",
      generationKwByGspId: { "1": "300", "2": "40", "3": "9999" }
    } as YieldRow,
    {
      datetimeUtc: "2023-12-05T12:30:00+00:00",
      generationKwByGspId: { "1": "200", "2": "20", "3": "9999" }
    } as YieldRow,
    {
      datetimeUtc: "2023-12-05T12:00:00+00:00",
      generationKwByGspId: { "1": "100", "2": "10", "3": "9999" }
    } as YieldRow
  ];

  const wire = (inDay: YieldRow[] | undefined) => {
    mockedLoad.mockImplementation((url: unknown) => {
      if (typeof url === "string" && url.includes("regime=in-day")) {
        return { data: inDay, isLoading: false, isValidating: false, error: undefined };
      }
  ...
  test("sums only the selected GSP ids, in kW, per timestamp", () => {
    const rows = [
      {
        datetimeUtc: "2023-12-05T12:00:00+00:00",
        generationKwByGspId: { "1": "100", "2": "10", "3": "9999" }
    ...
    const { result } = renderHook(() => useGetGspData(["1", "2"]));

    expect(result.current.pvRealDataIn).toEqual([
      { datetimeUtc: "2023-12-05T12:00:00+00:00", solarGenerationKw: 110 },
      { datetimeUtc: "2023-12-05T12:30:00+00:00", solarGenerationKw: 220 }
    ]);
  });

  test("ids with no entry, and unparseable values, are skipped rather than poisoning the sum", () => {
    wire([
      {
        datetimeUtc: "2023-12-05T12:00:00+00:00",
        generationKwByGspId: { "1": "100", "2": "not-a-number" }
      } as YieldRow
    ]);

    const { result } = renderHook(() => useGetGspData(["1", "2", "77"]));
    expect(result.current.pvRealDataIn).toEqual([
      { datetimeUtc: "2023-12-05T12:00:00+00:00", solarGenerationKw: 100 }
    ]);
  });

  test("an empty or undefined response aggregates to an empty array", () => {
    wire([]);
    expect(renderHook(() => useGetGspData(["1"])).result.current.pvRealDataIn).toEqual([]);

    wire(undefined);
    expect(renderHook(() => useGetGspData(["1"])).result.current.pvRealDataIn).toEqual([]);
  });

  test("reverse-chronological data is reordered oldest-first in the output", () => {
    wire(descendingRows());
    const { result } = renderHook(() => useGetGspData(["1", "2"]));

    expect(result.current.pvRealDataIn.map((d: { datetimeUtc: string }) => d.datetimeUtc)).toEqual([
  ...
  test("AUDIT B5: the raw SWR-cached response array is mutated in place", () => {
    // CHARACTERISATION: current behaviour is wrong — `pvDataRaw.reverse()`
    // reverses the array SWR is caching, so every other consumer of that cache
    // key sees the reordered array. It should copy (`[...pvDataRaw].reverse()`
    // or `toReversed()`) before reordering.
    //
    // WHEN B5 IS FIXED IN PHASE 4 THIS ASSERTION INVERTS: the input array must
    // then be unchanged, i.e. `expect(raw[0].datetimeUtc).toBe(newestFirst)`.
    const raw = descendingRows();
    const newestFirst = raw[0].datetimeUtc;
    expect(newestFirst).toBe("2023-12-05T13:00:00+00:00");

    wire(raw);
    renderHook(() => useGetGspData(["1", "2"]));

    expect(raw[0].datetimeUtc).toBe("2023-12-05T12:00:00+00:00");
    expect(raw[2].datetimeUtc).toBe("2023-12-05T13:00:00+00:00");
  });

  test("already-chronological data is left alone", () => {
    const raw = descendingRows().reverse();
    const oldestFirst = raw[0].datetimeUtc;
    wire(raw);
    renderHook(() => useGetGspData(["1", "2"]));
    expect(raw[0].datetimeUtc).toBe(oldestFirst);
  });

  test("AUDIT B5: the recorded actuals fixture is exactly the reverse-chronological shape that triggers the mutation", () => {
    const rows = allGspActualHistoricCompact as unknown as YieldRow[];
    expect(rows[0].datetimeUtc > rows[rows.length - 1].datetimeUtc).toBe(true);
  });
});
