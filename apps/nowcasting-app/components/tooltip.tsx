      ...
      <div className="absolute flex flex-col items-center hidden mb-8 group-hover:flex">
        <span
          className={`absolute ${getPositionClass(
            position,
          )} bottom-0 w-auto mb-1 z-10 p-2 text-xs leading-none text-white whitespace-no-wrap bg-mapbox-black shadow-lg`}
        >
          {tip}
        </span>
        <div className="w-3 h-3 -mt-2 rotate-45 bg-mapbox-black"></div>
      </div>
    </div>
  );
};
