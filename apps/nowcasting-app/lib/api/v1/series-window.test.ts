/**
 * The shared history window for region time-series requests.
 *
 * Moved here from `components/helpers/data.test.ts` on 2026-08-17, with the helper itself: the
 * window is applied by the query layer now (see `series-window.ts`), so it is tested next to the
 * thing that applies it rather than next to the map's value pipeline. Assertions unchanged.
 */
import { describe, expect, jest, afterEach, beforeEach, it } from "@jest/globals";
 ...
 */
describe("defaultSeriesStart (B2)", () => {
  beforeEach(() => {
    jest.useFakeTimers();
  ...
  describe("getEarliestForecastTimestamp — two days back, rounded DOWN to a 6-hour UTC boundary", () => {
    it.each([
      ...
      freeze(now);
      expect(defaultSeriesStart()).toBe(expected);
    });
    ...
    it.each(["00:00", "06:00", "12:00", "18:00"])(
      ...
      (hhmm) => {
        ...
        expect(defaultSeriesStart()).toBe(`2025-12-05T${hhmm}:00.000Z`);
      }
    );
  });
  ...
  describe("the viewer's timezone must not change the UTC window", () => {
    ...
    it.each(zones)("a viewer in %s gets the same window as a UK viewer (BST)", (zone) => {
      ...
      expect(defaultSeriesStart()).toBe("2025-07-13T12:00:00.000Z");
    });
    ...
    it.each(zones)("a viewer in %s gets the same window across their own DST change", (zone) => {
      ...
      expect(defaultSeriesStart()).toBe("2025-11-01T12:00:00.000Z");
    });
  });
});
