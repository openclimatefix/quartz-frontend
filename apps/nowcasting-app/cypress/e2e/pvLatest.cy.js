...
describe("Load the page", () => {
  ...
  it("matches the snapshot", () => {
    ...
    cy.intercept("/v0/solar/GB/national/forecast?historic=false&only_forecast_values=true&model_name=blend&trend_adjuster_on=true&UI=true", {
      fixture: "national_forecast.json"
    }).as("getNationalForecast");
    cy.intercept("/v0/solar/GB/national/pvlive?regime=in-day&UI=true", {
      fixture: "national_pvlive.json"
    }).as("getNationalPvLive");

    cy.intercept(
      "v0/solar/GB/gsp/forecast/all/?compact=true?compact=true&start_datetime_utc*&end_datetime_utc*&UI=true",
      {
    ...
    cy.intercept("/v0/solar/GB/gsp/pvlive/all?compact=true&end_datetime_utc*&UI=true", {
      fixture: "gsp_pvlive_history.json"
    }).as("getGspPvLiveHistory");
    cy.intercept(
      "/v0/solar/GB/gsp/pvlive/all?regime=in-day&compact=true&compact=true&start_datetime_utc*&UI=true",
      {
    ...
    cy.intercept(
      "/v0/solar/GB/national/forecast?forecast_horizon_minutes=240&historic=true&only_forecast_values=true&UI=true",
      { fixture: "national_4_hour_forecast.json" }
    ).as("getNational4HourForecast");

    cy.intercept("/v0/system/GB/gsp/?UI=true", { fixture: "gsp_list.json" }).as("getGspList");
